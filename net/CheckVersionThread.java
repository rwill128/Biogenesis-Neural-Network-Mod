...
public class CheckVersionThread extends Thread {
	...
	public CheckVersionThread(Component frame) {
		parentFrame = frame;
	}
	...
	public void run() {
		...
		try {
			...
			JOptionPane.showMessageDialog(parentFrame,Messages.getInstance().getString("T_YOUR_VERSION")+translateVersion(String.valueOf(Utils.VERSION))+"\n"+Messages.getInstance().getString("T_LAST_VERSION")+s,  //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ 
					Messages.getInstance().getString("T_VERSION_TITLE"), JOptionPane.INFORMATION_MESSAGE);   //$NON-NLS-1$
			br.close();
		} catch (MalformedURLException e) {
			e.printStackTrace();
		} catch (IOException e) {
			JOptionPane.showMessageDialog(parentFrame,Messages.getInstance().getString("T_CANT_CHECK_CURRENT_VERSION")+e.getLocalizedMessage(), //$NON-NLS-1$ 
					Messages.getInstance().getString("T_VERSION_TITLE"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
		} finally {
			if (br != null)
				try {
					br.close();
