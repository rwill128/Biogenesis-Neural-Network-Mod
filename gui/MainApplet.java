...
public class MainApplet extends JApplet implements ActionListener {
	...
	@Override
	public void init() {
		launchButton = new JButton(Messages.getInstance().getString("T_START")); //$NON-NLS-1$
	...
	public void actionPerformed(ActionEvent evt) {
		if (mainWindow == null) {
			mainWindow = new Biogenesis().start();
			launchButton.setText(Messages.getInstance().getString("T_STOP")); //$NON-NLS-1$
			mainWindow.addWindowListener(new WindowAdapter() {
            @Override
			public void windowClosed(WindowEvent wevt) {
            	releaseApplication();
            	launchButton.setText(Messages.getInstance().getString("T_START")); //$NON-NLS-1$
