...
public class ParamDialog extends JDialog {
	...
	public ParamDialog(Frame parent) {
		...
		super(parent,Messages.getInstance().getString("T_PARAMETERS_CONFIGURATION"),true); //$NON-NLS-1$
	...
	protected void setComponents() {
		...
		defaultButton = new JButton(Messages.getInstance().getString("T_DEFAULT_VALUES")); //$NON-NLS-1$
		...
		// Set up general tab
		tabbedPane.addTab(Messages.getInstance().getString("T_GENERAL"), setGeneralTab()); //$NON-NLS-1$
		// Set up world tab
		tabbedPane.addTab(Messages.getInstance().getString("T_WORLD"), setWorldTab()); //$NON-NLS-1$
		// Set up organisms tab
		tabbedPane.addTab(Messages.getInstance().getString("T_ORGANISMS"), setOrganismsTab()); //$NON-NLS-1$
		// Set up metabolism tab
		tabbedPane.addTab(Messages.getInstance().getString("T_METABOLISM"), setMetabolismTab()); //$NON-NLS-1$
		// Set up genes tab
		tabbedPane.addTab(Messages.getInstance().getString("T_GENES"), setGenesTab()); //$NON-NLS-1$
	...
	protected JPanel setGeneralTab() {
		...
		JLabel label = new JLabel(Messages.getInstance().getString("T_LANGUAGE")); //$NON-NLS-1$
		panel.add(label);
		localeCombo = new JComboBox(Messages.getSupportedLocalesNames());
		localeCombo.setSelectedIndex(Messages.getInstance().getLocaleIndex());
		panel.add(localeCombo);
		generalPanel.add(panel);
		//Frames per second
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_TIME_PER_FRAME")); //$NON-NLS-1$
		panel.add(label);
		delayText = new JTextField(Integer.toString(Utils.DELAY),6);
		panel.add(delayText);
		label = new JLabel(Messages.getInstance().getString("T_MILLISECONDS")); //$NON-NLS-1$
		panel.add(label);
		generalPanel.add(panel);
		// OpenGL
		panel = new JPanel();
		...
		panel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED),
				Messages.getInstance().getString("T_HARDWARE_ACCELERATION"))); //$NON-NLS-1$
		hardwareNoneRadio = new JRadioButton(Messages.getInstance().getString("T_NONE")); //$NON-NLS-1$
		hardwareOpenGLRadio = new JRadioButton(Messages.getInstance().getString("T_OPENGL")); //$NON-NLS-1$
		hardwareFBObjectCheck = new JCheckBox(Messages.getInstance().getString("T_DISABLE_FBOBJECT")); //$NON-NLS-1$
		...
		panel.add(new JLabel(Messages.getInstance().getString("T_DIRECTX_IS_AUTOMATICALLY_DETECTED_AND_INITIALIZED"))); //$NON-NLS-1$
		panel.add(new JLabel(Messages.getInstance().getString("T_APPLICATION_MUST_BE_RESTARTED_TO_APPLY_CHANGES"))); //$NON-NLS-1$
		generalPanel.add(panel);
		return generalPanel;
	}
	...
	protected JPanel setWorldTab() {
		...
		// World size
		JPanel panel = new JPanel();
		JLabel label = new JLabel(Messages.getInstance().getString("T_WIDTH")); //$NON-NLS-1$
		panel.add(label);
		widthText = new JTextField(Integer.toString(Utils.WORLD_WIDTH),6);
		panel.add(widthText);
		label = new JLabel(Messages.getInstance().getString("T_HEIGHT")); //$NON-NLS-1$
		panel.add(label);
		...
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_INITIAL_OXYGEN")); //$NON-NLS-1$
		panel.add(label);
		initialO2Text = new JTextField(Double.toString(Utils.INITIAL_O2),6);
		panel.add(initialO2Text);
		label = new JLabel(Messages.getInstance().getString("T_INITIAL_CARBON_DIOXIDE")); //$NON-NLS-1$
		panel.add(label);
		...
		// Rubbing - Elasticity
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_RUBBING_COEFFICIENT")); //$NON-NLS-1$
		panel.add(label);
		rubbingText = new JTextField(Double.toString(Utils.RUBBING),6);
		panel.add(rubbingText);
		worldPanel.add(panel);
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_ELASTICITY_COEFFICIENT")); //$NON-NLS-1$
		panel.add(label);
	...
	protected JPanel setOrganismsTab() {
		...
		JPanel panel = new JPanel();
		JLabel label = new JLabel(Messages.getInstance().getString("T_INITIAL_NUMBER")); //$NON-NLS-1$
		panel.add(label);
		initialnumberText = new JTextField(Integer.toString(Utils.INITIAL_ORGANISMS),6);
		panel.add(initialnumberText);
		label = new JLabel(Messages.getInstance().getString("T_INITIAL_ENERGY")); //$NON-NLS-1$
		panel.add(label);
		...
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_LIFE_EXPECTANCY")); //$NON-NLS-1$
		panel.add(label);
		maxageText = new JTextField(Integer.toString(Utils.MAX_AGE),6);
		panel.add(maxageText);
		label = new JLabel(Messages.getInstance().getString("T_MUTATION_PERCENTAGE")); //$NON-NLS-1$
		panel.add(label);
		...
		// Upkeep cost
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_UPKEEP_COST_DIVISOR")); //$NON-NLS-1$
		panel.add(label);
		...
		// Drain substance
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_SUBSTANCE_DRAINAGE_DIVISOR")); //$NON-NLS-1$
		panel.add(label);
		...
		// Decay energy
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_DECAY_ENERGY")); //$NON-NLS-1$
		panel.add(label);
	...
	protected JPanel setMetabolismTab() {
		...
		JPanel panel = new JPanel();
		JLabel label = new JLabel(Messages.getInstance().getString("T_PHOTOSYNTHETIC_METABOLISM")); //$NON-NLS-1$
		label.setAlignmentX(Component.CENTER_ALIGNMENT);
		metabolismPanel.add(label);
		metabolismPanel.add(Box.createVerticalStrut(10));
		// Obtained energy
		label = new JLabel(Messages.getInstance().getString("T_OBTAINED_ENERGY_DIVISOR")); //$NON-NLS-1$
		panel.add(label);
		...
		label = new JLabel(Messages.getInstance().getString("T_CHEMOORGANOTROPHIC_METABOLISM")); //$NON-NLS-1$
		label.setAlignmentX(Component.CENTER_ALIGNMENT);
		metabolismPanel.add(Box.createVerticalStrut(10));
		metabolismPanel.add(label);
		metabolismPanel.add(Box.createVerticalStrut(10));
		// Obtained energy
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_OBTAINED_ENERGY")); //$NON-NLS-1$
		panel.add(label);
		...
		// Released energy
		panel = new JPanel();
		label = new JLabel(Messages.getInstance().getString("T_RELEASED_ENERGY_PROPORTION")); //$NON-NLS-1$
		panel.add(label);
	...
	protected JPanel setGenesTab() {
		...
		genesPanel.add(new JLabel(Messages.getInstance().getString("T_COLOR2"),SwingConstants.CENTER)); //$NON-NLS-1$
		genesPanel.add(new JLabel(Messages.getInstance().getString("T_PROBABILITY"),SwingConstants.CENTER)); //$NON-NLS-1$
		genesPanel.add(new JLabel(Messages.getInstance().getString("T_COST"),SwingConstants.CENTER)); //$NON-NLS-1$
		
		label = new JLabel(Messages.getInstance().getString("T_RED"),SwingConstants.CENTER); //$NON-NLS-1$
		...
		label = new JLabel(Messages.getInstance().getString("T_GREEN"),SwingConstants.CENTER); //$NON-NLS-1$
		genesPanel.add(label);
		...
		label = new JLabel(Messages.getInstance().getString("T_BLUE"),SwingConstants.CENTER); //$NON-NLS-1$
		genesPanel.add(label);
		...
		label = new JLabel(Messages.getInstance().getString("T_CYAN"),SwingConstants.CENTER); //$NON-NLS-1$
		genesPanel.add(label);
		...
		label = new JLabel(Messages.getInstance().getString("T_WHITE"),SwingConstants.CENTER); //$NON-NLS-1$
		genesPanel.add(label);
		...
		label = new JLabel(Messages.getInstance().getString("T_GRAY"),SwingConstants.CENTER); //$NON-NLS-1$
		genesPanel.add(label);
		...
		label = new JLabel(Messages.getInstance().getString("T_YELLOW"),SwingConstants.CENTER); //$NON-NLS-1$
		genesPanel.add(label);
	...
	void checkParams() {
		int i;
		double d;
		if (Messages.getInstance().getLocaleIndex() != localeCombo.getSelectedIndex())
			Messages.getInstance().setLocale(localeCombo.getSelectedIndex());
		try {
